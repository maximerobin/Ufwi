...
static int multi_dispatch(struct ufwiclient_plugin_t *plugin, unsigned int event_id, nuauth_session_t * session, const char *arg);
...
static int multi_send_packet(struct ufwiclient_plugin_t * plugin, conn_t * arg)
{
...
static int send_connected(nuauth_session_t *session, sec_nuauth_session_t * ssession, char * state)
{
	...
	ret = ufwissl_write(session->ufwissl, buf, ntohs(header->length));
	if (ret < 0) {
		if (session->verbose)
