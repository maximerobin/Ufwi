...
enum {
	...
	UFWISSL_INIT_ERR,	     /** NuSSL initialisation failed */
...
#define ufwiclient_error ufwiclient_error_t

typedef struct nuauth_session nuauth_session_t;

/* libufwiclient return code structure */
typedef struct {
	ufwiclient_error_family_t family;
	int error;
} ufwiclient_error_t;
