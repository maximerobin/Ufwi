...
int assert_conf_vars(struct llist_head *l)
{
	...
	var_str = ufwibase_config_table_get(l, "section/var_in_section");
	if (var_str == NULL || strcmp(var_str,"str") != 0) {
		fprintf(stderr, "Failed test: ufwibase_config_table_get(l, \"section/var_in_section\")\n");
		fprintf(stderr, "  var is '%s'\n", var_str);
		return 1;
	}

	/* var_global => "global" */
	var_str = ufwibase_config_table_get(l, "var_global");
	if (var_str == NULL || strcmp(var_str,"global") != 0) {
		fprintf(stderr, "Failed test: ufwibase_config_table_get(l, \"var_global\")\n");
		fprintf(stderr, "  var is '%s'\n", var_str);
		return 1;
	}
