...
int ufwissl_hash_file(ufwissl_hash_algo_t algo, const char * filename,
		    unsigned char *out, size_t *outsz)
{
	...
	switch (algo) {
		case UFWISSL_HASH_MD5:
			md = EVP_md5();
			break;
		case UFWISSL_HASH_SHA1:
			md = EVP_sha1();
			break;
		case UFWISSL_HASH_SHA256:
			md = EVP_sha256();
			break;
		case UFWISSL_HASH_SHA512:
			md = EVP_sha512();
			break;
		default:
			return -1;
...
int ufwissl_hash_compute(ufwissl_hash_algo_t algo, const char *data, size_t datasz, char *out, size_t *outsz)
{
	return ufwissl_hash_compute_with_salt(algo, data, datasz, NULL, 0, out, outsz);
}

int ufwissl_hash_compute_with_salt(ufwissl_hash_algo_t algo, const char *data, size_t datasz, const char *salt, size_t saltsz, char *out, size_t *outsz)
{
	const EVP_MD *md;
	EVP_MD_CTX mdctx;
