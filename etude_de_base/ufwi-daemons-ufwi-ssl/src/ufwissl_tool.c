...
{
	fprintf(stdout, "ufwissl_tool [-hVAPCLv]\n"
...
int main(int argc, char **argv)
{
	...
	while ((option = getopt_long(argc, argv, options_list, long_options, NULL)) != -1) {
		...
		case 'h':
			...
			return 0;
		case 'V':
			fprintf(stdout, "ssl_tool for ufwissl version %s\n", PACKAGE_VERSION);
			return 0;
		case 'A':
	...
	switch (command) {
	case COMMAND_VERIFY:
		rc = ufwissl_local_check_certificate(cert, ca_cert, ca_path, crl,
				ret_message, message_sz);
		fprintf(stdout, "ufwissl_local_check_certificate: %d\n"
				"message: %s\n",
				rc, ret_message);
		break;
	default:
