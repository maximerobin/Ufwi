...
/**
 * Clean mutex, memory, etc. before exiting ufwi-filterd
 */
...
void nufw_hard_cleanup(int signal)
{
	...
	log_area_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_FATAL,
			"[+] ufwi-filterd \"hard\" cleanup (catch double signal)");
	nufw_prepare_quit();
	log_area_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_FATAL,
			"[+] Exit ufwi-filterd");
	exit(EXIT_SUCCESS);
...
void nufw_cleanup(int signal)
{
	...
	log_area_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_FATAL,
			"[+] Stop ufwi-filterd (catch signal)");
	nufw_prepare_quit();
	log_area_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_FATAL,
			"[+] Exit ufwi-filterd");
	exit(EXIT_SUCCESS);
...
int nufw_use_config()
{
	...
	value = nufw_config_table_get("tls_cacert");
	if (value != NULL && ca_file == NULL) {
		ca_file = strdup(value);
	}

	value = nufw_config_table_get_or_default("tls_cert",DEFAULT_NUFW_CERT);
	if (value != NULL && cert_file == NULL) {
		cert_file = strdup(value);
	}

	value = nufw_config_table_get_or_default("tls_key",DEFAULT_NUFW_KEY);
	if (value != NULL && key_file == NULL) {
		key_file = strdup(value);
	}

	value = nufw_config_table_get("tls_crl");
	if (value != NULL && crl_file == NULL) {
		crl_file = strdup(value);
	}

	value = nufw_config_table_get_or_default("destination", AUTHREQ_ADDR);
 ...
 *   - Run main loop
 *
 * When ufwi-filterd is running, main loop and two threads (packetsrv() and
...
int main(int argc, char *argv[])
{
	...
	if (tls.session) {
		char buf[256];
		buf[0] = '\0';
		ufwissl_session_get_cipher(tls.session, buf, sizeof(buf));
	...
	log_area_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_FATAL,
			"[+] ufwi-filterd " NUFW_FULL_VERSION " started");

	if (daemonize == 0) {
		log_area_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_CRITICAL,
				"ufwi-filterd launched in foreground (without -D option), "
