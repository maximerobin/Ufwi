...
static void tls_activity_cb(struct ev_loop *loop, ev_io *w, int revents) {
	...
	if (revents & EV_READ) {
		...
		do {
			...
			ret = authsrv(NULL);
		} while ((ret != NU_EXIT_ERROR) && (ufwissl_read_available(tls.session)));
 ...
 * on this link. Check if packet useful for ufwi-filterd. If yes, add it to packet
...
#endif
	fd = ufwissl_session_get_fd(tls.session);
	if (fd >= 0) {
		ev_io_init(&tls.ev_io, tls_activity_cb,
				ufwissl_session_get_fd(tls.session), EV_READ);
...
int auth_request_send(uint8_t type, struct queued_pckt *pckt_data)
{
	...
	if (ufwissl_write(tls.session, (char*)data, msg_length) < 0) {
		debug_log_printf(DEBUG_AREA_MAIN, DEBUG_LEVEL_DEBUG,
				 "Error during ufwissl_write (auth_request_send).");
		shutdown_tls();
		log_area_printf(DEBUG_AREA_GW,
				DEBUG_LEVEL_WARNING,
				"[!] TLS send failure");
